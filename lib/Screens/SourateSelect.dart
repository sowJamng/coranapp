  ...
  void _save(index) async{
   ...
   //favorie.date=DateFormat.yMMMd().format(DateTime.now());
      ...
      //  favor.setVerset(widget.sourate.versets[index]);
       ...
       if(await helper.exist(favor.getNumverset(),favor.getNumsourate())==false)
         { 
        ...
        //  _showAletDialog('Statut', 'Problem d\'Ajout aux favories');
               ...
               _showSnackBar(context, 'Verset ${favor.getNumverset()} déjà ajouté aux favories');
  ...
  Widget build(BuildContext context) {
   ...
   if(this._searching==false)DropdownButton(
                      ...
                      underline:Text('${widget.sourate.numero}'+'.'+widget.sourate.nom,
                          style: TextStyle(color: Colors.white,fontSize: 11.0),overflow: TextOverflow.ellipsis) ,                         
                    ),
      ...
      WillPopScope(
        onWillPop: () {
          setState(() {
             ...
             posi= itemPositionsListener.itemPositions.value.isNotEmpty?itemPositionsListener.itemPositions.value.first.index:6;  
          });
                  ...
                  child: Container(
                    ...
                    // child: Scrollbar(                       
                      ...
                      itemScrollController: itemScrollController,
                        ...
                        itemCount: versetsearch.length,//widget.sourate.versets.length,
                        itemBuilder: (BuildContext context, int index) {
                          final verset  = versetsearch[index];//widget.sourate.versets[index];
    ...
    ((widget.sourate==aladiyat)&&(verset.numv==9)))    
           ...
           Container(
            margin: EdgeInsets.only(left: 35.0,right: 3.0),
                            padding: EdgeInsets.only(left:35.0,right: 3.0),

             child:
               Align(                        
                               alignment: Alignment.topRight,
                               child: Text(//'﴾${verset.numv}﴿'+'\t '+
                               verset.arabe ,
                               //'﴾١﴿',//NumberFormat.currency(locale: 'ar').format(index),  
                                  style: TextStyle(                                  
                                     fontSize: _height,
                                     fontFamily: 'mequran',//'amiriregquran',                                
                               ...
                               alignment: Alignment.bottomRight,
                                child: 
                                Text((widget.sourate==alalaq&&verset.numv==19)||(widget.sourate==araf&&verset.numv==206)||(widget.sourate==arrad&&verset.numv==15)||
                               (widget.sourate==annahl&&verset.numv==49)||(widget.sourate==alisra&&verset.numv==109)||(widget.sourate==maryam&&verset.numv==58)||
                               (widget.sourate==alhadjj&&(verset.numv==18||verset.numv==77))||(widget.sourate==furqan&&verset.numv==60)||(widget.sourate==annaml&&verset.numv==26)||
                               (widget.sourate==asajdah&&verset.numv==15)||(widget.sourate==sad&&verset.numv==15)||(widget.sourate==fussila&&verset.numv==37)||
                               (widget.sourate==annajm&&verset.numv==62)||(widget.sourate==inshiqaq&&verset.numv==21) ?
                    ...
                    Text(
                                ...
                                Row(
                                  ...
                                  children: <Widget>[
                   IconButton(
                         icon:Icon(Icons.play_arrow,color: Colors.green,), //isplay?Icon(Icons.pause_circle_outline,color:Colors.indigo):Icon(Icons.play_arrow,color: Colors.green,),
                                    onPressed:(){print(vr);
                                    ...
                                    //   }
                                    // });
                                     isPlaying?stop(vr):play(vr);//playautomat(index);
                                    }                                                           
                                   
                                    ),
                                   IconButton(
                                  icon:Icon(Icons.pause_circle_outline, color:Colors.indigo),//Color(0xFFE8582E)),
                                  onPressed: isPlaying ? () => pause() : null,
                                    ),
                                  
               IconButton(icon:  favori,
                                  color:Colors.indigo, 
                                  onPressed: (){
                                    setState(() {
                                    _save(index);
                                     _showSnackBar(context, 'Verset ${verset.numv} ajouté aux favories avec succés');
