...
class DatabaseHelperSourate {
  ...
  factory DatabaseHelperSourate(){  //on utilise factory pour que lle constructeur puissse retourner une valeur
    if(_databaseHelper==null){
    _databaseHelper= DatabaseHelperSourate._createInstance(); // il sera execute qu une seule fois 
    }
 ...
 Future<Database> initializeDatabase() async{
    ...
    var sourateDatabase= await openDatabase(path,version:1,onCreate:_createDb);
    return sourateDatabase;
  }

  void _createDb(Database db,int version) async{

  await db.execute('CREATE TABLE $souraTable($colId INTEGER PRIMARY KEY AUTOINCREMENT,$colDate TEXT,$colNumsourate INTEGER,$colOntap INTEGER ,$colNumverset INTEGER)');
 ...
 Future<int> insertSourate( SourateCourante sourate) async{
    print('insertion sourate courante');
    Database db=await this.database;
  ...
  Future<int> updateSourate( SourateCourante sourate) async{
    Database db=await this.database;
    var result= await db.update(souraTable, sourate.toMap(),where: '$colId=?',whereArgs: [sourate.id]);
    return result;
  }
...
Future<bool> exist(int nums,int numv) async{
   ...
   Database db=await this.database;
    var result= await db.rawQuery('SELECT * FROM $souraTable WHERE $colNumsourate=$nums and $colNumverset=$numv');
  ...
  Future<int> deleteAll()async{
    ...
    print('suppression de tout les enregistrement');
    return result;
  }
   
  Future<List<Map<String,dynamic>>> getsourate(int id) async{
    Database db=await this.database;
    var result= await db.rawQuery('SELECT * FROM $souraTable WHERE $colId=$id');
    return result;
  }
