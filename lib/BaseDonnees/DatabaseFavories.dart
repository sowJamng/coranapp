...
class DatabaseHelper {
  ...
  factory DatabaseHelper(){  //on utilise factory pour que lle constructeur puissse retourner une valeur
    if(_databaseHelper==null){
    _databaseHelper= DatabaseHelper._createInstance(); // il sera execute qu une seule fois 
    }
 ...
 Future<Database> initializeDatabase() async{
    ...
    var favoriDatabase= await openDatabase(path,version:1,onCreate:_createDb);
    return favoriDatabase;
  }

  void _createDb(Database db,int version) async{

  await db.execute('CREATE TABLE $favorieTable($colId INTEGER PRIMARY KEY AUTOINCREMENT,$colDate TEXT,$colVarabe String,$colVwolof String,$colNumverset INTEGER,$colNomsourate String,$colNumsourate INTEGER,$colOntap INTEGER )');
...
Future<int> verify(int numverset,int numsourate) async{
    Database db=await this.database;
    var exist=await db.rawQuery('select $colId from $favorieTable WHERE $colNumverset= $numverset and $colNumsourate=$numsourate');
    if(exist.length==0)
    return 0;
    else
    return 1;
  ...
  Future<bool> exist(int numverset,int numsourate) async{
    Database db=await this.database;
    var exist=await db.rawQuery('select * from $favorieTable WHERE $colNumverset= $numverset and $colNumsourate=$numsourate');
    if(exist.length==0)
    return false;
    else
    return true;
  ...
  Future<int> updateFavori( Allfavories favori) async{
    Database db=await this.database;
    var result= await db.update(favorieTable, favori.toMap(),where: '$colId=?',whereArgs: [favori.id]);
    return result;
  }

  Future<int> delete(int id)async{
    Database db=await this.database;
    int result= await db.rawDelete('DELETE FROM $favorieTable WHERE $colId=$id');
    return result;
  }

  Future<List<Map<String,dynamic>>> getFavori(int id) async{
    Database db=await this.database;
    var result= await db.rawQuery('SELECT * FROM $favorieTable WHERE $colId=$id');
    return result;
  }
